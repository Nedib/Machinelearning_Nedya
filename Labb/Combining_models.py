"""This code provides a structured framework for evaluating and selecting models by first preparing the data, splitting it into training and testing sets, and defining evaluation metrics. It then performs hyperparameter tuning and model training using grid search and cross-validation techniques. The results include the identification of the best parameters for each model and detailed evaluation reports, enabling informed decision-making in selecting the most effective algorithm and hyperparameters for the given dataset. Overall, it offers a systematic approach to model evaluation and selection, ensuring robust performance assessment and optimization"""
...
# Pipeline för KNN med StandardScaler
pipe_KNN = Pipeline([
    ("scaler", StandardScaler()),
    ("knn", KNeighborsClassifier())
])

# Pipeline för logistisk regression med StandardScaler
pipe_log_reg = Pipeline([
    ("scaler", StandardScaler()),
    ("logistic", LogisticRegression(max_iter=500, solver='saga'))
])

# Pipeline för Random Forest med StandardScaler
pipe_RFC = Pipeline([
    ("scaler", StandardScaler()),
...
def perform_grid_search_and_train(model_name, model_pipeline, param_grid, X_train, y_train, X_val, y_val):
    ...
    evaluate_model(best_model, X_val, y_val, f"Classification report for {model_name}:")
...
evaluate_model(knn_grid_search.best_estimator_, X_test, y_test, "KNN")
print("Best Logistic Regression Model:")
evaluate_model(log_reg_grid_search.best_estimator_, X_test, y_test, "Logistic Regression")
print("Best Random Forest Model:")
evaluate_model(rfc_grid_search.best_estimator_, X_test, y_test, "Random Forest")